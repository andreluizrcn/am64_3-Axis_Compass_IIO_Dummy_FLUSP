...
static const struct iio_chan_spec iio_dummy_channels[] = {
    ...
    /* Adding Magnometer channels to support one channel per axis*/
    {
        .type = IIO_MAGN,
        .modified = 1,
        .channel2 = IIO_MOD_X,
     ...
     */
    {
        ...
        .info_mask_separate = BIT(IIO_CHAN_INFO_RAW) |
                              /*
                               * Internal bias and gain correction values.
                               * Applied by the hardware or driver prior to
                               * userspace seeing the readings. Typically part
    ...
    },
    {
        ...
        .info_mask_shared_by_type =
            BIT(IIO_CHAN_INFO_ENABLE) | BIT(IIO_CHAN_INFO_CALIBHEIGHT),
...
static int iio_dummy_write_raw(struct iio_dev *indio_dev,
  ...
  case IIO_CHAN_INFO_PROCESSED:
    switch (chan->type) {
    case IIO_STEPS:
      scoped_guard(mutex, &st->lock) { st->steps = val; }
      return 0;
    case IIO_ACTIVITY:
      if (val < 0)
        val = 0;
  ...
  case IIO_CHAN_INFO_CALIBSCALE: {
    ...
    for (i = 0; i < ARRAY_SIZE(dummy_scales); i++)
      if (val == dummy_scales[i].val && val2 == dummy_scales[i].val2)
        break;
  ...
  case IIO_CHAN_INFO_CALIBBIAS:
    scoped_guard(mutex, &st->lock) { st->accel_calibbias = val; }
    return 0;
  case IIO_CHAN_INFO_ENABLE:
    switch (chan->type) {
    case IIO_STEPS:
      scoped_guard(mutex, &st->lock) { st->steps_enabled = val; }
      return 0;
    default:
      return -EINVAL;
    }
